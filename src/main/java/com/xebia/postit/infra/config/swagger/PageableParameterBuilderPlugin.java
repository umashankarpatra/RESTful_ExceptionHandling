package com.xebia.postit.infra.config.swagger;
...
public class PageableParameterBuilderPlugin implements OperationBuilderPlugin {
    ...
    /**
     * Page name may be varied. See
     * {@link org.springframework.data.web.PageableHandlerMethodArgumentResolver#setPageParameterName(String)}
     *
    ...
    /**
     * Size name may be varied. See
     * {@link org.springframework.data.web.PageableHandlerMethodArgumentResolver#setSizeParameterName(String)}
     *
    ...
    /**
     * Sort name may be varied. See
     * {@link org.springframework.data.web.SortHandlerMethodArgumentResolver#setSortParameter(String)}
     *
    ...
    protected Parameter createPageParameter(ParameterContext context) {
        ...
        return new ParameterBuilder().name(getPageName()).parameterType(PAGE_TYPE).modelRef(intModel)
    ...
    protected Parameter createSizeParameter(ParameterContext context) {
        ModelReference intModel = createModelRefFactory(context).apply(resolver.resolve(Integer.TYPE));
        return new ParameterBuilder().name(getSizeName()).parameterType(SIZE_TYPE).modelRef(intModel)
    ...
    protected Parameter createSortParameter(ParameterContext context) {
        ...
        return new ParameterBuilder().name(getSortName()).parameterType(SORT_TYPE).modelRef(stringModel)
                .allowMultiple(true).description(SORT_DESCRIPTION).build();
    }
    ...
    protected Function<ResolvedType, ModelReference> createModelRefFactory(ParameterContext context) {
        ModelContext modelContext = inputParam(context.getGroupName(),
                context.resolvedMethodParameter().getParameterType(), context.getDocumentationType(),
